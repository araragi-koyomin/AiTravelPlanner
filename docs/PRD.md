...
#### 2.5.1 数据同步
- 基于 Supabase Realtime 实现实时数据同步
- 支持行程/行程项/费用三层订阅
- 支持 INSERT/UPDATE/DELETE 全量事件同步
- 多设备实时数据同步

#### 2.5.2 同步状态管理
- 同步状态指示器（SyncStatusIndicator）
  - 同步中（蓝色旋转动画）
  - 已同步（绿色对勾）
  - 同步失败（红色警告+错误信息）
- 待同步操作徽章（PendingSyncBadge）
- 操作队列分组展示
...
用户需自行申请并配置第三方服务的 API Key，用于启用相关功能。所有 Key 均加密存储在 Supabase 数据库中。
...
**高德地图**
- API Key（必填）
- 安全密钥 SecurityJsCode（可选，推荐配置）
- 用于地图展示和路线规划
- 申请地址：https://lbs.amap.com/

> **安全说明**：公开部署时必须使用用户自行配置的 API Key，避免将开发者 Key 暴露在前端代码中。
