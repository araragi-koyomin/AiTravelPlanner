...
- [x] 语音记录费用
- [x] 行程导出（PDF/图片）
- [x] 云端数据同步（Supabase Realtime）
- [x] API Key 设置页面（支持多凭证类型：智谱AI、科大讯飞三字段、高德地图双字段）

**P2 - 可选功能（可以有）**
- [ ] 第三方登录（Google、GitHub）
- [x] 行程收藏功能
- [x] 主题切换（深色/浅色，已实现设置保存）
- [x] 多语言支持（中文/英文，已实现设置保存）
- [ ] 通知功能
...
**Task 7.1 - 功能测试**
- [x] 11大模块功能全覆盖验证（认证、行程生成/编辑/展示、地图、语音、费用、同步、导出、设置）
- [x] 测试报告（test_report/Task7-测试与Bug修复测试报告.md）

**Task 7.2 - Bug修复**
- [x] TypeScript 错误修复（16 个）
- [x] ESLint 错误修复（65 个）
- [x] 测试失败修复（34+ 个用例）
- [x] 测试挂起问题修复（Promise 超时、Realtime mock、内存溢出）
- [x] 测试目录结构整合（src/integration/ → src/tests/integration/）

**Task 7.3 - 单元测试补充**
- [x] 工具函数单元测试（exportUtils 437+ tests 等）
- [x] 服务层单元测试（auth, itinerary, expense, settings, ai, realtime 等）
- [x] 组件单元测试（pages, components, hooks 共 ~70 个测试文件）
- [x] 集成测试（6个文件，94个用例）
- [x] 测试覆盖率 ~75%（目标 > 60%）
- [x] 验收报告（test_report/Task7-测试与Bug修复验收报告.md）

#### 📋 待开发（第8周 - 部署与文档）

**Task 8.0 - Edge Function 用户 API Key 支持**
- [ ] 修改 Edge Functions 支持读取用户配置的 API Key

**Task 8.1 - Docker 部署配置**
- [ ] 完善 Dockerfile + docker-compose.yml + nginx 配置

**Task 8.2 - GitHub Actions CI/CD**
- [ ] 自动构建、推送镜像、自动化部署流程

**Task 8.3 - 文档完善**
- [ ] README 补充部署说明、API 文档、用户手册

**Task 8.4 - 最终测试与提交**
- [ ] 全量功能检查、代码规范、打包镜像、部署验证
...
├── src/                       # 源代码
│   ├── test/                  # 共享测试基础设施（setup.ts/factories/mocks）
│   ├── tests/integration/     # 集成测试（6个文件，94个用例）
│   ├── components/            # 公共组件（ui/layout/map/itinerary/voice/expense/export）
│   │   ├── ui/              # 基础UI组件
│   │   │   ├── Button.tsx   # 按钮组件
│   │   │   ├── Input.tsx    # 输入框组件
│   │   │   ├── Card.tsx     # 卡片组件
...
│   │   └── voice/           # 语音组件
│   │       ├── VoiceInput.tsx # 语音输入主组件
│   │       ├── VoiceButton.tsx # 录音按钮组件
│   │       ├── VoiceVisualizer.tsx # 音量可视化组件
│   │       └── VoiceResult.tsx # 识别结果组件
│   │   └── expense/         # 费用组件
│   │       ├── BudgetOverview.tsx # 预算概览组件
│   │       ├── BudgetProgressBar.tsx # 预算进度条组件
│   │       ├── ExpensePieChart.tsx # 支出饼图组件
│   │       ├── ExpenseBarChart.tsx # 支出柱状图组件
│   │       ├── OverBudgetAlert.tsx # 超支提醒组件
│   │       └── ExpenseAnalysis.tsx # 费用分析组件
│   │   └── export/          # 导出组件
│   │       ├── ExportButton.tsx # 导出按钮组件
│   │       └── ExportModal.tsx # 导出弹窗组件
│   ├── pages/               # 页面组件
│   │   ├── Home.tsx        # 首页
│   │   ├── Login.tsx       # 登录页
│   │   ├── Register.tsx    # 注册页
...
│   ├── stores/                # Zustand 状态管理（authStore/itineraryEditStore/itineraryListStore/syncStore）
│   ├── hooks/                 # 自定义 Hooks（useAMap/useVoiceRecognition/useRealtime 等）
│   ├── utils/                 # 工具函数（cn/crypto/date/mapUtils/voiceParser/exportUtils 等 12 个）
│   │   ├── cn.ts           # 类名合并工具
│   │   ├── crypto.ts       # 加密工具
│   │   ├── date.ts         # 日期工具
│   │   ├── mapUtils.ts     # 地图工具函数
│   │   ├── voiceParser.ts  # 语音解析工具
│   │   ├── expenseUtils.ts # 费用工具函数
...
├── supabase/                  # Supabase 数据库脚本与 Edge Functions
│   ├── migrations/            # 数据库迁移脚本（001-014）
│   ├── seeds/                 # 种子数据
│   └── functions/             # Edge Functions（generate-itinerary/optimize-itinerary 等 7 个）
├── test_report/               # 测试报告和验收报告（Task1.1 ~ Task7，共 32+ 份）
│   ├── Task1.1 ~ Task1.4      # 基础设施阶段报告
│   ├── Task2.1 ~ Task2.10     # 核心功能阶段报告
│   ├── Task3.2 ~ Task3.3      # 增强功能阶段报告
│   └── Task7                  # 测试与Bug修复报告 + 验收报告
├── .trae/                   # 项目规则和模板
│   ├── rules/               # 项目规则
│   │   └── project_rules.md
│   └── templates/           # Git 模板
...
- ✅ 用户提供的 API Key 加密存储在数据库中（AES-256）
...
**公开部署须知**：
- ⚠️ 如果要将应用公开部署（如 GitHub Pages），**必须**让用户自行配置 API Key
- ⚠️ 前端代码中无法隐藏 API Key，任何硬编码的 Key 都会被恶意用户获取
- ✅ 本项目已实现用户 API Key 管理功能，支持多凭证类型配置
